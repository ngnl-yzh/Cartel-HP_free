...
- 한국어 월 표현도 다음 규칙으로 변환하세요 (연도는 문맥 추론):
  - "N월 초"       → YYYY-N-01
  - "N월 상순"     → YYYY-N-01
  - "N월 중순"     → YYYY-N-15
  - "N월 중"       → YYYY-N-15
  - "N월 말"       → 해당 월의 말일 (예: 7월 말 → YYYY-07-31, 2월 말 → YYYY-02-28)
  - "N월 하순"     → 해당 월의 말일
  - "N월 말~M월 초" 범위 → 마감이면 M월 01일, 시작이면 N월 말일
...
- **review_dates**: 단계별 심사 일정. **특정 날짜(YYYY-MM-DD 변환 가능)가 있을 때만 포함.** 없으면 [].
  - 【공모전 정보】의 "심사기간" → {"label": "심사", "date": "심사기간 시작일"}
  - 본문의 "1차 심사", "서류 심사", "발표 심사" 등 각 단계별 날짜가 확정될 때만 추가
  - "7월 말", "8월 중순", "추후 공지" 등 확정 날짜 없으면 **절대 포함하지 마세요**.
...

{
  ...
  "review_dates": [{"label": "심사 단계명", "date": "YYYY-MM-DD"}],
  "prize": "시상 내역 요약",
  "link": "공식 URL 또는 null",
